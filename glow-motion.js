(function attachGlowMotion(global) {
  ...
  function createTitleEffects(title) {
    ...
    const shadowPalettes = [
      ["rgba(35,94,68,.22)", "rgba(73,224,146,.11)"],
      ["rgba(42,47,72,.3)", "rgba(89,124,255,.15)"],
      ["rgba(136,79,18,.28)", "rgba(255,170,67,.14)"],
    ];
    ...
    let projectionRect = projection.getBoundingClientRect();
    let lineMetrics = [];
    let animationFrame = 0;
    let active = false;

    const measureLines = () => {
      projectionRect = projection.getBoundingClientRect();
      lineMetrics = projectedLines.map((element, lineIndex) => {
        const rect = element.getBoundingClientRect();
        const palette = shadowPalettes[lineIndex] || shadowPalettes[0];
        element.style.setProperty("--near-shadow-color", palette[0]);
        element.style.setProperty("--far-shadow-color", palette[1]);
        return {
          element,
    ...
    const writeShadows = () => {
      ...
      if (!lineMetrics.length) measureLines();
      lineMetrics.forEach((metric) => {
        ...
        deltaX /= distance;
        deltaY /= distance;
        const shadowLength = clamp(30 - distance * .012, 15, 28);

        element.style.setProperty("--shadow-x", `${(deltaX * shadowLength * .48).toFixed(2)}px`);
        element.style.setProperty("--shadow-y", `${(deltaY * shadowLength * .48).toFixed(2)}px`);
        element.style.setProperty("--shadow-far-x", `${(deltaX * shadowLength).toFixed(2)}px`);
        element.style.setProperty("--shadow-far-y", `${(deltaY * shadowLength).toFixed(2)}px`);
      });
    };

    let lastShadowFrame = 0;
    const followPointer = (time) => {
      if (time - lastShadowFrame < 32) {
